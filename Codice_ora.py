...
class SettimaPagina(QWidget):
    def __init__(self, callback_indietro, callback_avanti):
        super().__init__()
        self.setStyleSheet("background: transparent;")
        self.scelta_clima = ""                     # ← qui salviamo la scelta
        ...
        for i, testo in enumerate(climi):
            ...
            btn.clicked.connect(lambda _, t=testo: self.seleziona_clima(t))
    ...
    def seleziona(self, bottone_selezionato, testo):
        self.scelta_clima = testo
        for btn in self.bottoni:
            if btn == bottone_selezionato:
...
class OttavaPagina(QWidget):   # Regione / Stato
    def __init__(self, callback_indietro, callback_avanti):
        ...
        grid = QGridLayout()
        stati = ["Italia 🇮🇹", "Francia 🇫🇷", "Spagna 🇪🇸", "Grecia 🇬🇷", "Portogallo 🇵🇹", "Germania 🇩🇪", "Svizzera 🇨🇭", "Austria 🇦🇹", "Regno Unito 🇬🇧", "Altri"]
        self.bottoni_stato = []
        for i, testo in enumerate(stati):
            ...
            btn.clicked.connect(lambda _, t=testo: self.seleziona_regione(t))
            grid.addWidget(btn, i // 2, i % 2)
...
class NonaPagina(QWidget):   # Paesaggio
    def __init__(self, callback_indietro, callback_avanti):
        super().__init__()
        self.setStyleSheet("background: transparent;")
        ...
        for i, testo in enumerate(paesaggi):
            ...
            btn.setFixedSize(160, 55)
            btn.setStyleSheet("background-color: white; color: #000080; font-weight: bold; border-radius: 10px;")
            btn.clicked.connect(lambda _, t=testo: self.seleziona_paesaggio(t))
            grid.addWidget(btn, i // 2, i % 2)
...
class DecimaPagina(QWidget):   # Mood
    def __init__(self, callback_indietro, callback_avanti):
        super().__init__()
        self.setStyleSheet("background: transparent;")
        self.scelta_mood = ""
        ...
        for i, testo in enumerate(mood_list):
            ...
            btn.setStyleSheet("background-color: white; color: #000080; font-weight: bold; border-radius: 10px;")
            btn.clicked.connect(lambda _, t=testo: self.seleziona_mood(t))
            grid.addWidget(btn, i // 2, i % 2)
...
class FinestraPrincipale(QStackedWidget):
    def __init__(self):
        super().__init__()
        percorso = "PYTHON\\OOP (program. ad oggetti)\\LABORATORIO (Lecci)\\LIBRERIE GRAFICHE (PyQt5)\\PROGETTO (gruppo2)\\sfondo_app.png"
        self.setStyleSheet(f"FinestraPrincipale {{background-color: #1e3a8a; border-image: url('{percorso}') 0 0 0 0 stretch stretch; }}")
        ...
        self.pag11 = PaginaRisultati(self.vai_a_pag10, self.vai_a_pag1)   # ← RISULTATI

        for p in [self.pag1, self.pag2, self.pag3, self.pag4, self.pag5,
                  self.pag6, self.pag7, self.pag8, self.pag9, self.pag10, self.pag11]:
            self.addWidget(p)
    ...
    def vai_a_pag11(self):   # ← QUI parte l'algoritmo
        if not self.pag6.scelta_clima or not self.pag8.scelta_paesaggio or not self.pag9.scelta_mood:
            QMessageBox.warning(self, "Attenzione", "Seleziona clima, paesaggio e mood prima di continuare!")
            return
        top3 = self.calcola_meta_perfecta()
        self.pag11.mostra_risultato(top3, self.pag2.input_nome.text())
        self.setCurrentIndex(10)

    # ====================== ALGORITMO A PUNTEGGI ======================
    def raccogli_risposte_utente(self):
        return {
            'nome': self.pag2.input_nome.text().strip(),
            'budget_max': self.pag4.slider.value(),
            'num_persone': self.pag5.spin_total.value(),
            'num_bambini': self.pag5.spin_bambini.value(),
            'disabilita': self.pag5.check_disabilita.isChecked(),
            'clima': self.pag6.scelta_clima,
            'paesaggio': self.pag8.scelta_paesaggio,
            'mood': self.pag9.scelta_mood,
            'esperienze': [cb.text() for cb in self.pag10.checks if cb.isChecked()],
        }
